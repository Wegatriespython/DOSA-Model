...
def maximize_utility(savings, current_wage, current_price, historical_price, working_hours, discount_factor=0.95, periods=20, alpha=0.5, linear_solver='ma27'):
    ...
    if result is not None:
        optimal_consumption, optimal_working_hours, optimal_savings = result
        last_solution = (optimal_consumption, optimal_working_hours)  # Update last_solution for warm start
...
def _maximize_utility(savings, current_wage, current_price, historical_price, working_hours, discount_factor, periods, alpha, linear_solver='ma27'):
    ...
    # Variables
    model.consumption = pyo.Var(domain=pyo.PositiveReals, initialize=1.0, bounds=(1e-6, None))
    model.working_hours = pyo.Var(domain=pyo.NonNegativeReals, bounds=(0, 15.99999), initialize= min(working_hours,16))
    model.leisure = pyo.Var(domain=pyo.PositiveReals, bounds=(1e-6, 16))
    ...
    model.future_consumption = pyo.Var(domain=pyo.PositiveReals, bounds=(1e-6, None))
    ...
    def new_savings_constraint(model):
        return model.new_savings == model.savings + model.current_wage * model.working_hours - (model.consumption * model.current_price)

    @model.Constraint()
    def future_consumption_constraint_lower(model):
        return model.future_consumption >= model.new_savings / (model.current_price * model.periods) + 1e-6

    @model.Constraint()
    def future_consumption_constraint_upper(model):
        return model.future_consumption >= 0.01

    @model.Constraint()
    def budget_constraint(model):
        return model.savings + model.current_wage * model.working_hours >= model.consumption * model.current_price
    ...
    try:
        ...
        if (results.solver.status == pyo.SolverStatus.ok and
            ...
            return optimal_consumption, optimal_working_hours, optimal_savings
        else:
